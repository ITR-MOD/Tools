...
from image_utils import remove_blue_channel, save_image
...
def process_image(file_path):
    try:
        ...
        no_blue = remove_blue_channel(img)
        
        # Save the processed image
        output_path = f"{os.path.splitext(file_path)[0]}.no-GB.png"
        save_image(no_blue, output_path)
...
def main():
    if len(sys.argv) <= 1:
        print("Usage: python remove_green_blue.py path/to-image.png")
        return
